...
class UserLoginSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if not user:
            raise AuthenticationFailed(detail="Unable to log in with provided credentials.", code="authorization")

        if not getattr(user, "is_active", True):
            raise AuthenticationFailed(detail="User account is disabled.", code="authorization")
